...
2. 속성(Property) : 선택자에 적용할 스타일의 속성을 정의
3. 속성값(Property Value) : 속성에 할당할 구체적인 값을 설정

### 20.2 작성 규칙
* `{`: 스타일 적용 시작
* `속성: 속성값;`: 속성에 값을 설정 (여러 속성을 ;로 구분)
* `}`: 스타일 적용 종료
* 주석은 `/* */`로 작성

## 21. CSS 적용
### 21.1 인라인 방식
* HTML 태그 내 `style` 속성을 사용해 직접 스타일을 지정
* 소규모 테스트나 특수 목적에 유용
* 스타일이 길어지면 복잡도가 증가, 가상 클래스(:hover)와 가상 요소(::before) 스타일 적용 불가
* 일반적인 실무에서는 권장되지 않음

### 21.2 내부 스타일
* HTML 파일 내 `<style>` 태그를 사용해 스타일을 정의
* 코드의 재사용성과 가독성 향상
* HTML 파일이 길어질수록 복잡도 증가
* HTML 파일 내에서 직접 스타일을 작성하고 관리

### 21.3 외부 스타일
* `<link>` 태그를 사용해 외부 CSS 파일을 연결합니다.
* HTML 파일과 CSS 파일을 분리하여 관리할 수 있어 효율적.
* `rel`: 스타일 시트의 관계를 정의 (예: `stylesheet`).
* `href`: 참조할 CSS 파일의 위치를 지정.
* 실무에서 가장 많이 사용
